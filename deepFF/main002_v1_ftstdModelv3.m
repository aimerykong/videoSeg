...
modelName = 'noPSP_v3_standardConv2_softmax_net-epoch-55.mat';
...
for i = 317:numel(netbasemodel.layers)            
    for j = 1:length(netbasemodel.layers(i).paramIndexes)
        ...
        if j == 3
            netbasemodel.params(curInd).learningRate = 0.00;
        else
...
netbasemodel.layers(netbasemodel.getLayerIndex('res6_bn')).block.bnorm_moment_type_trn = 'global';
...
opts.expDir = fullfile('./exp', 'main002_v1_ftstdModelv3');
