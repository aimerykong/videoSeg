...
def getPrediction( args, groundTruthFile, predFolderName='./' ):
    ...
    if not args.predictionPath:
        ...
        if 'CITYSCAPES_RESULTS' in os.environ:
            ...
            #rootPath = os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..','result_val_predEval')
            rootPath = os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..',predFolderName)
        print(rootPath)
    ...
    #print(len(args.predictionWalk[0])) # 2 as below
    #print(len(args.predictionWalk[0][0])) # root folder
    #print(len(args.predictionWalk[0][1])) # image list -- 500
    ...
    for root, filenames in args.predictionWalk:
        ...
        #print(filePattern)
        #print(filenames)
        #print(fnmatch.filter(filenames, filePattern))
...
if 'CITYSCAPES_DATASET' in os.environ:
    ...
    args.cityscapesPath = os.path.join(os.path.dirname(os.path.realpath(__file__)),'..','..','..','dataset')
...
args.groundTruthSearch  = os.path.join( args.cityscapesPath , "cityscapes_gtFine" , "gtFine", "val" , "*", "*_gtFine_labelIds.png" )
...
def main(argv):
    ...
    if (len(argv) > 3):
        ...
        for gt in groundTruthImgList:
            predictionImgList.append( getPrediction(args,gt,predFolderName) )
